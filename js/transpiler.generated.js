...
// source-hash: 0ea3abfe62959332d429b9b26d707405d5983dfb
let wasm;
 ...
 * @returns {string}
 */
export function transpile(content, maybe_compiler_options) {
  let deferred3_0;
  let deferred3_1;
  try {
    ...
    wasm.transpile(retptr, ptr0, len0, addHeapObject(maybe_compiler_options));
 ...
 *   exports: { transpile: typeof transpile }
 * }>}
 */
export async function instantiateWithInstance(opts) {
  const { instance } = await loader.load(
    opts?.url ?? new URL("transpiler_bg.wasm", import.meta.url),
    opts?.decompress,
  );
